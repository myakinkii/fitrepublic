sap.ui.define([ "ru/fitrepublic/shared/controller/BaseController", "ru/fitrepublic/shared/appMgr",
...
], function (Controller, AppMgr, JSONModel, PurchaseDlg, NotesDlg) {
	"use strict";
	...
	return Controller.extend("ru.fitrepublic.ui5_coachApp.controller.Purchase", {
		...
		formatPurchaseTitle:function(name,gym,ptype,description,tmplName){
			if (ptype!='S') return name+" @"+(description||gym);
			else return tmplName+ " ("+name+")";
		},
		...
		goToTemplate:function(){
			var id=this.getView().getBindingContext("odata").getProperty("templateId");
		...
		displayNotes:function(){
			var clientId=this.getView().getBindingContext("odata").getProperty("owner_id");
