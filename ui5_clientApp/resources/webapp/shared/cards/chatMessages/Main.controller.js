	...
	return Controller.extend("ru.fitrepublic.cards.chatMessages.Main", {
		...
		onInit: function () {
			...
			this.getOwnerComponent().initPromise.then(function(data){
				var firstChannel=data.results[0] && data.results[0].channelId;
				mdl.setData({
					channels:data.results,
					messages:{},
					selectedChannel:firstChannel
				});
				if (firstChannel) this.setMessages(mdl,firstChannel);
		...
		navToPurchaseChat:function(){
			...
			var channel=mdlData.channels.filter(function(c){ return c.channelId==mdlData.selectedChannel; });
			var purchId=channel[0]&&channel[0].id;
			if (purchId) card.fireAction({
				parameters:{
					dst:"purchase",
